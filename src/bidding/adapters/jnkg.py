...
_BASE_URL = "https://dzzb.jnkgjtdzzbgs.com"

# 晋能控股招标采购平台（与宁夏 nxgyzb 同一 CMS 平台）。
# 列表页：/cms/default/webfile/<cat>/index.html
# 详情页：/cms/default/webfile/<cat>/<yyyymmdd>/<snowflake-id>.html
# ⚠️ 待联网校验：各分类对应的实际公告类型需在站点导航上确认。
_CATEGORIES: dict[str, tuple[str, NoticeType]] = {
    "1ywgg1": ("招标公告", NoticeType.BID_ANNOUNCEMENT),
    "2ywgg1": ("采购公告", NoticeType.NON_BID_ANNOUNCEMENT),
    # TODO(联网校验): 补充 1ywgg2~8、2ywgg2~8 的公告类型（中标/变更/候选人/废标等）
}
...
@register
class JnkgAdapter(SiteAdapter):
    meta = AdapterMeta(
        name="jnkg",
        display_name="晋能控股招标采购",
        base_url=_BASE_URL,
    ...
    async def scrape_list(
        ...
        for cat in cats:
            ...
            logger.info("jnkg.scrape_category", notice_type=notice_type.value, category=cat, label=label)
            page_no = 1
            while True:
                url = (
                ...
                try:
                    ...
                    logger.warning("jnkg.goto_failed", url=url)
                    break
                items = await self._extract_list_items(page, cat)
                logger.info("jnkg.list_page", category=cat, page=page_no, items=len(items))
                if not items:
                    break
                for item in items:
    ...
    async def _extract_list_items(self, page: Page, cat: str) -> list[dict]:
        return await page.evaluate(
            """([cat]) => {
                ...
                const seen = new Set();
                const out = [];
                document.querySelectorAll(`a[href*="/cms/default/webfile/${cat}/"]`).forEach(a => {
    ...
    async def _fetch_detail(self, page: Page, url: str) -> BidNotice | None:
        ...
        try:
            ...
            logger.debug("jnkg.detail_failed", url=url[:80])
            return None
        finally:
